...
registerChannelAdapter('whatsapp', {
  factory: () => {
    ...
    async function getNormalizedGroupMetadata(jid: string): Promise<GroupMetadata | undefined> {
      ...
      // Modern WhatsApp groups use addressingMode: 'lid' — Baileys' internal
      // session/sender-key lookups expect the original participant identifiers
      // (LIDs). Do NOT translate participant IDs here — translation is for
      // OUR consumers, not Baileys'.
    ...
    async function connectSocket(): Promise<void> {
      ...
      // Cast to any: event was renamed/removed in Baileys 7. Handler still
      // attaches if present, no-op otherwise; translateJid handles resolution.
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
